...
const DEFAULT_ADMIN = {
    email: "boichitrobdshop@gmail.com",
    password: "Uhanna09" // Default beginner-friendly credential
};
...
// 12. INITIALIZATION
document.addEventListener("DOMContentLoaded", () => {
    // Only run admin-panel logic on actual admin pages (login form or dashboard sidebar present).
    // Without this guard, this block ran on every public page (index/products/offers/checkout/product)
    // and redirected visitors with no admin session to a relative "login.html" — a path that only
    // exists as admin/login.html — causing a 404 on Vercel for anyone without a saved admin session.
    const isAdminPage = !!(document.getElementById("adminLoginForm") || document.getElementById("adminSidebar"));
