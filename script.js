...
// CATALOG SYNC ENGINE
// ---------------------------------------------------------------------
// DEFAULT_PRODUCTS_DATA (in this file) is the ONLY source of truth for
// the product catalog. The localStorage copy exists purely as an
// in-browser cache/mirror of it — nothing in this site ever lets a
// customer or admin edit product data through the UI, so the cache
// should never "win" over the code.
//
// To detect ANY change made in the code (an edit, a removed product, a
// newly added product) we compute a lightweight fingerprint of
// DEFAULT_PRODUCTS_DATA on every page load. If that fingerprint doesn't
// match the one stored alongside the cached copy in localStorage, the
// cache is stale and gets fully overwritten from DEFAULT_PRODUCTS_DATA.
// This never touches boichitroCart or boichitro_db_orders.
...
function getDBProducts() {
    ...
    try {
        ...
        // DEFAULT_PRODUCTS_DATA fingerprint we have right now. If the code
        // was edited (product added/edited/removed) since that copy was
        // saved, storedVersion !== CATALOG_VERSION and we fall through to
        // a full resync below — this is what fixes the stale-data bug.
        if (data && storedVersion === CATALOG_VERSION) {
            const parsed = JSON.parse(data);
            if (Array.isArray(parsed) && parsed.length > 0) {
                _dbProductsCache = parsed;
    ...
    // No cache, corrupted cache, or DEFAULT_PRODUCTS_DATA changed in the
    // code since the last save — DEFAULT_PRODUCTS_DATA is authoritative,
    // so (re)seed localStorage from it entirely. This naturally makes
    // removed products disappear, edited fields update, and new products
    // appear, without ever touching cart/order data.
...
function saveDBProducts(products) {
    try {
        // Deep-clone so this saved snapshot can't be mutated later by
        // accidental in-place edits to DEFAULT_PRODUCTS_DATA elsewhere.
        const cloned = JSON.parse(JSON.stringify(products));
        localStorage.setItem(STORAGE_KEYS.PRODUCTS, JSON.stringify(cloned));
        localStorage.setItem(STORAGE_KEYS.PRODUCTS_VERSION, CATALOG_VERSION);
        // Update live PRODUCTS_DATA variable for legacy compatibility
        PRODUCTS_DATA = cloned;
        _dbProductsCache = cloned;
    } catch (e) {
